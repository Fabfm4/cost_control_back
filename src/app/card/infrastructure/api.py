...
async def list_card_router():
    return CollectionModel(data=await list_card(CardDB.query))
...
async def create_card_router(card: CardModelMandatoryRequest = Body(...)):
    return await create_card(card, CardDB.create, CardDB.query)
...
async def get_card_router(pk: str):
    try:
        ...
        return await get_card(
            _id, CardDB.query, raise_404_error)
